<?php
    ...
    //Database Queries
    $sqlLog = "SELECT V.VN, V1, TS FROM log AS l NATURAL JOIN vitals AS V WHERE l.USR='{$currentUser}' AND l.RPID='{$rpi}' AND TYP='ST' AND (V.VN='{$vitalOne}' OR V.VN='{$vitalTwo}') ORDER BY l.TS ASC;"; //Select all logs related to the current user & 
    ...
    $arrayLogTSOne = array(); //This array will contain all the timestamps according to the dateStart/End, timeStart/End, and timeInterval
    $arrayLogTSTwo = array(); //This array will contain all the timestamps according to the dateStart/End, timeStart/End, and timeInterval
    if(mysqli_num_rows($resultLog) > 0) { 
        while($row = mysqli_fetch_assoc($resultLog)) {
            ...
            $tempTS = $vitalTS->format('Y-m-d H:i');

            if($vitalName == $vitalOne) { $arrayLogVitalOne[] = $tempVital; $arrayLogTSOne[] = $tempTS; } 
            else { $arrayLogVitalTwo[] = $tempVital; $arrayLogTSTwo[] = $tempTS; }
        }
    }

    //Output
    echo "<canvas class='charts-canvas' id='primary-chart' style='width: content-box;'></canvas>";
    echo "<script>createchart('primary-chart', 'line', '{$vitalOne}'," .  json_encode($arrayLogTSOne) .",". json_encode($arrayLogVitalOne) . ")</script>";
    if($vitalOne != "Exhaust"){
        echo "<canvas class='charts-canvas' id='secondary-chart' style='width: content-box;'></canvas>";
        echo "<script>createchart('secondary-chart', 'line', '{$vitalTwo}'," .  json_encode($arrayLogTSTwo) .",". json_encode($arrayLogVitalTwo) . ")</script>";
    }
?>