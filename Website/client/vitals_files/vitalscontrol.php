<?php
    ...
    // foreach($listOfSelectedOptions as $lOSO) {
    //     echo "{$lOSO} <br>";
    // }
    ...
    for($counter_i = 0; $counter_i < $amtOfRPis; $counter_i++) {
        ...
        while($counter_j != ($counter_i + $tempOffset)) {
            ...
            $sqlUpdate = "UPDATE status SET VV='{$tempVV}' WHERE VID='{$tempVID}' AND RPID='{$tempRPi}' AND USR='{$currentUser}';";
            ...
            $counter_j += 1; //Increment counter_j
        }
    }
